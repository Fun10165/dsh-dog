...
import { AtomicVerifierRegistry } from '../src/verifiers.ts'
...
describe('debugger snapshot', () => {
  it('never presents an old run against a newer graph revision', async () => {
    ...
    expect(trace.events.map(event => event.kind)).toEqual([
      'goal_started', 'verifier_started', 'verifier_passed', 'goal_settled',
    ])
  ...
  it('exposes an in-flight verifier without mixing runtime context into trusted evidence', async () => {
    ...
    await writeFile(join(root, 'artifact.txt'), 'verified artifact')
    const repository = new DogRepository(join(root, '.dog-store'))
    const verifiers = new AtomicVerifierRegistry()
    ...
    verifiers.register({
      ...
      verify: async () => {
        enteredVerifier?.()
        await release
        return { passed: true, observation: { checked: true } }
      },
    })
    ...
    expect(liveTrace.events.map(event => event.kind)).toEqual(['goal_started', 'verifier_started'])
    ...
    expect(completedTrace.events.map(event => event.kind)).toEqual([
      'goal_started', 'verifier_started', 'verifier_passed', 'goal_settled',
    ])
  ...
  it('records a structured verifier error and preserves the honest node result', async () => {
    ...
    expect(trace.events.map(event => event.kind)).toEqual([
      'goal_started', 'verifier_started', 'goal_error', 'goal_settled',
    ])
    expect(trace.events.find(event => event.kind === 'goal_error')?.error).toMatchObject({
