...
import type { CiReport, CompiledGraph, DogRun, GoalResult, JsonValue } from '../core/model.ts'

/**
 * The session facts tools read. The Host supplies them from the Session Store
 * so this module never reaches into a live Agent's internals.
 */
export interface DogSessionFacts {
 /** Session working directory at creation time. */
 readonly cwd?: string
 /** Durable parent session id, when this session is a delegated child. */
...
export function createDogTools(
 ...
 sessionFacts: (sessionId: string) => DogSessionFacts | undefined = () => undefined,
 ...
 return [
  ...
  defineTool({
   ...
   async execute(args, exec) {
    exec.signal.throwIfAborted()
    const callerFacts = exec.agent === undefined ? undefined : sessionFacts(String(exec.agent.id))
    const run = await engine().startRun(args.graphId, {
     invocation: {
      callId: String(exec.callId),
      ...(exec.agent === undefined ? {} : { agentSessionId: String(exec.agent.id) }),
      ...(callerFacts?.parentSessionId === undefined ? {} : { parentSessionId: callerFacts.parentSessionId }),
     },
  ...
  defineTool({
   ...
   async execute(args, exec) {
    ...
    const facts = sessionFacts(String(exec.agent.id))
    const binding = await engine().bindAgent({
     runId: args.runId,
     goalId: args.goalId,
     role: args.role,
     sessionId: String(exec.agent.id),
     ...(facts?.parentSessionId === undefined ? {} : { parentSessionId: facts.parentSessionId }),
    })
    exec.signal.throwIfAborted()
    return jsonResult({
